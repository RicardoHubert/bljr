									 ...
									 <table class="table table-hover data">
										...
										<tbody>
												...
												<tr>
													...
													<td><img style="height: 50px;" src="logo/{{$ormawa->logo_ormawa}}"/></td>
													<td><a href="{{action('OrmawaController@profile',$ormawa->id)}}">{{$ormawa->nama_ormawa}}</td></a>
													...
													@if(auth()->user()->role == 'admin')
													<td>
														<a href="{{action('OrmawaController@edit', $ormawa->id)}}"  class="btn btn-sm btn-warning">Edit</a>
														<a href="{{action('OrmawaController@delete', $ormawa->id)}}"  class="btn btn-sm btn-danger" onclick="return confirm('Jika data ini dihapus maka dapat menghilangkan seluruh kegiatan didalamnya, Apakah anda yakin ingin menghapus data ini??')">Delete</a>
